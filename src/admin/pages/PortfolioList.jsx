...
const PortfolioList = () => {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const res = await api.adminPortfolios.list(token);
  ...
  const handleDelete = async (slug) => {
    ...
    try {
      await api.adminPortfolios.remove(token, slug);
      load();
    } catch (err) {
      alert(err.message);
    }
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-3">
          <Link to="/admin/portfolio/categories">
            <Button variant="outline">Kategori</Button>
          </Link>
          <Link to="/admin/portfolio/create">
            <Button variant="primary">Tambah Portofolio</Button>
          </Link>
        </div>
      </div>
      ...
      <DataTable
        columns={[
          ...
          { Header: 'Industri', accessor: 'industry' },
          {
            Header: 'Aksi',
            accessor: 'actions',
            Cell: (row) => (
              <div className="flex gap-2">
                <Link to={`/admin/portfolio/${row.slug}/edit`} className="text-cyan">
                  Edit
                </Link>
