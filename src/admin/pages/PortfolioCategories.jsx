...
const PortfolioCategories = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await api.adminPortfolios.categories.create(token, { name, slug: slug || undefined });
      setName('');
      setSlug('');
      load();
    } catch (err) {
  ...
  const handleDelete = async (id) => {
    ...
    await api.adminPortfolios.categories.remove(token, id);
    load();
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-semibold text-white">Kategori Portofolio</h1>
      </div>
